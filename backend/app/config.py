...
class Config:
    ...
    LEAVE_ADJUSTED_ATTENDANCE_ENABLED = _env_bool(
    ...
    JWT_SECRET_KEY = _JWT_SECRET_KEY or (
    ...
    STRICT_JWT_SECRET = _env_bool(
        "STRICT_JWT_SECRET", ENV in {"production", "prod", "staging"}
    )
    ...
    JWT_ACCESS_TOKEN_EXPIRES = _env_int(
    ...
    # Use "redis://localhost:6379" for distributed
    ...
    PASSWORD_RESET_OTP_TTL_MINUTES = _env_int(
        "PASSWORD_RESET_OTP_TTL_MINUTES", 10
    )
    PASSWORD_RESET_OTP_MAX_ATTEMPTS = _env_int(
        "PASSWORD_RESET_OTP_MAX_ATTEMPTS", 5
    )

    # Session Security
    SECURE_SESSION_TIMEOUT_MINUTES = _env_int(
        "SECURE_SESSION_TIMEOUT_MINUTES", 30
    )
    ...
    HSTS_ENABLED = _env_bool(
        "HSTS_ENABLED", ENV in {"production", "prod", "staging"}
    )
    ...
    # Error Tracking
    ERROR_TRACKING_ENABLED = _env_bool(
        "ERROR_TRACKING_ENABLED", ENV in {"production", "prod", "staging"}
    )
    ...
    TASK_QUEUE_REDIS_URL = os.getenv(
    ...
    TASK_QUEUE_BASE_BACKOFF_SECONDS = _env_int(
        "TASK_QUEUE_BASE_BACKOFF_SECONDS", 10
    )
    TASK_QUEUE_MAX_BACKOFF_SECONDS = _env_int(
        "TASK_QUEUE_MAX_BACKOFF_SECONDS", 300
    )
    TASK_QUEUE_BACKOFF_JITTER_RATIO = _env_float(
        "TASK_QUEUE_BACKOFF_JITTER_RATIO", 0.25
    )
    TASK_QUEUE_RUNNING_TIMEOUT_SECONDS = _env_int(
        "TASK_QUEUE_RUNNING_TIMEOUT_SECONDS", 900
    )
    ACTIVE_SESSION_TIMEOUT_MINUTES = _env_int(
        "ACTIVE_SESSION_TIMEOUT_MINUTES", 180
    )
    ...
    TRAINER_ARTIFACT_RETENTION_DAYS = _env_int(
        "TRAINER_ARTIFACT_RETENTION_DAYS", 30
    )
