...
class ErrorTracker:
    ...
    @classmethod
    def track_error(
        cls, error, error_type="exception", user_id=None, context=None
    ):
        """
        ...
        try:
            ...
            logger.error(
                "Failed to track error to database: %s", e, exc_info=True
            )
    ...
    def track_validation_error(
        cls, message, field=None, value=None, user_id=None
    ):
    ...
    def get_recent_errors(cls, hours=24, limit=100):
        ...
        try:
            ...
            return list(
                errors_collection.find({"timestamp": {"$gte": cutoff}})
                .sort("timestamp", -1)
                .limit(limit)
            )
        except Exception:
    ...
    def handle_exception(func):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                user_id = (
                    getattr(g, "current_user", {}).get("_id")
...
def register_error_handlers(app):
    ...
    def internal_error(error):
        ...
        return ErrorHandler.api_error_response(
            "Internal server error", 500, error_id
        )
    ...
    def handle_mongo_error(error):
        ...
        app.logger.error(
            f"MongoDB error: {str(error)}", extra={"error_id": error_id}
        )
        return ErrorHandler.api_error_response("Database error", 500, error_id)
    ...
    def handle_generic_exception(error):
        ...
        error_id = ErrorTracker.track_error(error, user_id=user_id)
        return ErrorHandler.api_error_response(
            "An unexpected error occurred", 500, error_id
        )
