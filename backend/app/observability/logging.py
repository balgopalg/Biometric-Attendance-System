...
class StructuredLogger:
    ...
    def _log(self, level, message, **kwargs):
        ...
        if merged_data:
            ...
            log_message = json.dumps({"message": message, **merged_data})
        else:
            log_message = message
...
def configure_logging(app, log_level="INFO", log_format="text"):
    ...
    def log_request_summary(response):
        if hasattr(g, "request_started_at") and isinstance(
            g.request_started_at, datetime
        ):
            duration_ms = (
                datetime.now(timezone.utc) - g.request_started_at
            ).total_seconds() * 1000
            app.logger.info(
                "request_complete",
                extra={
