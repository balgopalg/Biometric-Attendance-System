...
def list_papers(user):
    ...
    for paper in papers:
        ...
        if (
            academic_year
            and _normalise_year(item.get("academic_year")) != academic_year
        ):
            continue
        if q and not (
...
def add_paper(user):
    ...
    if (
        not d.get("name")
        or not d.get("code")
        or not d.get("course_id")
        or not d.get("semester")
    ):
        return (
            jsonify(
                {"error": "name, code, course_id and semester are required"}
            ),
            400,
        )

    course_id, semester, error = _normalise_course_semester(
        d.get("course_id"), d.get("semester")
    )
    if error:
...
def edit_paper(user, pid):
    ...
    log_action(
        ...
        details=f"Paper {pid}",
        rollback=(
            _rb_replace("academic", "papers", {"_id": pid}, previous)
            if previous
            else None
        ),
    )
...
def remove_paper(user, pid):
    ...
    log_action(
        ...
        details=f"Paper {pid}",
        rollback=(
            _rb_restore("academic", "papers", previous) if previous else None
        ),
    )
    _clear_query_cache()
...
def bulk_assign(user):
    ...
    user_ids = [
        _as_text(sid) for sid in (d.get("user_ids") or []) if _as_text(sid)
    ]
    ...
    paper_ids_for_students = [
        _as_text(pid) for pid in (d.get("paper_ids") or []) if _as_text(pid)
    ]
    if user_ids and paper_ids_for_students and not d.get("lecturer_id"):
        ...
        if not valid_paper_ids:
            return (
                jsonify({"error": "No active papers found for assignment"}),
                400,
            )
    ...
    if not paper_ids:
        return (
            jsonify(
                {"error": "paper_ids or (paper_id + user_ids) is required"}
            ),
            400,
        )
    ...
    if course_id:
        ...
        if _course_is_inactive(course):
            return (
                jsonify(
                    {"error": "Cannot assign papers to an inactive course"}
                ),
                409,
            )
