...
def reassign_course_entities(user):
    ...
    if not from_course_id or not to_course_id:
        return (
            jsonify({"error": "from_course_id and to_course_id are required"}),
            400,
        )
    if from_course_id == to_course_id:
        return (
            jsonify(
                {"error": "from_course_id and to_course_id must be different"}
            ),
            400,
        )
...
_AUDIT_EXCLUDED_ACTIONS = list(
    set(_AUDIT_EXCLUDED_ACTIONS + ["HEARTBEAT", "QUEUE_CHECK", "STATUS_CHECK"])
)
...
def list_audit_logs(user):
    ...
    if is_super_admin(user):
        ...
        if dept_filter_id:
            ...
            for u in users_col.find(
                {"department_id": dept_filter_id}, {"_id": 1}
            ):
        ...
        logs, total = get_audit_logs(
            page, per_page, filters, department_id=None
        )
    else:
        ...
        if user_dept_id:
            ...
            all_user_id_variants = []
            for u in users_col.find(
                {"department_id": user_dept_id}, {"_id": 1}
            ):
                all_user_id_variants.extend(_id_variants(u["_id"]))
            dept_user_filter = {
                "$or": [
    ...
    audit_user_ids = [
        ...
        or (
            (item.get("details") or {}).get("user_id")
        ...
        or (
            ...
            and (item.get("details") or {}).get("user_id")
        )
    ]
    ...
    for raw in logs:
        ...
        details_user_id = (
            (raw.get("details") or {}).get("user_id")
        ...
        target_user = (
            user_map.get(_as_text(target_user_id)) if target_user_id else None
        )

        item["actor_name"] = (actor or {}).get("name") or (
            "System" if str(actor_id).lower() == "system" else "Unknown User"
        )
        item["actor_email"] = (actor or {}).get("email") or ""
        item["role"] = (
            (actor or {}).get("role") or raw.get("role") or "unknown"
        )

        if target_user:
            item["target_type"] = (
                f"{target_user.get('name', 'Unknown')} ({target_user.get('role', 'user')})"
            )
        ...
        if raw_rollback_until and raw_rollback_until.tzinfo is None:
            raw_rollback_until = raw_rollback_until.replace(
                tzinfo=timezone.utc
            )

        now = datetime.now(timezone.utc)
        eligible = (
            bool(raw_rollback_payload)
            and not raw_rolled_back
            and bool(raw_rollback_until)
            and now <= raw_rollback_until
        )
        ...
        item["target_user"] = (
            _as_text(target_user_id) if target_user_id else None
        )
...
def rollback_audit_action(user, log_id):
    ...
    if audit_log.get("rolled_back"):
        return (
            jsonify({"error": "This action has already been rolled back"}),
            400,
        )
    ...
    if not rollback_payload:
        return (
            jsonify({"error": "Rollback not available for this action"}),
            400,
        )
    ...
    if not rollback_until:
        rollback_until = (raw_ts or datetime.now(timezone.utc)) + timedelta(
            days=1
        )
...
def exam_eligibility_summary(user):
    ...
    academic_session = _normalise_year(
    ...
    if dept_scope_id:
        ...
        if target_dept and target_dept.get("name"):
            ...
            name_matches = sanitise_many(
                list(all_courses_col.find({"department": target_name}))
            )
    ...
    base_profile_query = (
        {"course_id": {"$in": visible_course_ids}}
        if visible_course_ids
        else {"course_id": "never_match"}
    )
    ...
    for row in sessions_col.aggregate(
        ...
        if gid_paper:
            classes_happened_by_paper[gid_paper] = (
                classes_happened_by_paper.get(gid_paper, 0) + count
            )
    ...
    for profile in profiles:
        ...
        course_status = (
            _as_text((course_doc or {}).get("status") or "active").lower()
            or "active"
        )
        ...
        if department_filter:
            course_department = _as_text(
                (course_doc or {}).get("department") or ""
            )
    ...
    if student_match_ids and paper_match_ids:
        for override in overrides_col.find(
            ...
            key = (
                _as_text(override.get("user_id")),
                _as_text(override.get("paper_id")),
            )
            override_map[key] = {
    ...
    for (
        ...
        for pid in enrolled:
            ...
            if profile_created_at:
                if profile_created_at.tzinfo is None:
                    profile_created_at = profile_created_at.replace(
                        tzinfo=timezone.utc
                    )
                paper_sessions = sessions_by_paper.get(pid_text, [])
                classes_happened = sum(
                    1 for ts in paper_sessions if ts >= profile_created_at
                )
            else:
                classes_happened = int(
                    classes_happened_by_paper.get(pid_text, 0) or 0
                )

            attended = attendance_count_map.get((_as_text(uid), pid_text), 0)
            pct = (
                round((attended / classes_happened) * 100, 2)
                if classes_happened > 0
                else 0.0
            )
            ...
            override_status = (
                None if not override else override.get("override_status")
            )
            override_reason = (
                "" if not override else override.get("reason", "")
            )
            ...
            per_paper_rows.append(
                {
                    ...
                    "course_department": (course or {}).get("department")
                    or "",
        ...
        overall_pct = (
            round((total_attended_overall / total_classes_overall) * 100, 2)
        ...
        for row in per_paper_rows:
            ...
            final_eligible = (
                ...
                else bool(override_status)
            )
            if final_eligible is None:
                eligibility_status = "no_lectures_yet"
            else:
                eligibility_status = (
                    "eligible" if final_eligible else "ineligible"
                )
    ...
    payload = {
        ...
        "ineligible_count": sum(
            1 for x in items if x["final_eligible"] is False
        ),
        "items": items,
    }
...
def set_exam_eligibility_override(user):
    ...
    if isinstance(raw_status, str):
        ...
        if raw_lower not in {"1", "0", "true", "false", "yes", "no", "y", "n"}:
            return (
                jsonify({"error": "override_status must be true or false"}),
                400,
            )
...
def set_exam_eligibility_override_bulk(user):
    ...
    return (
        jsonify(
            {
                "message": "Bulk eligibility overrides updated",
                "updated": len(unique_pairs),
            }
        ),
        200,
    )
...
def list_leave_requests(user):
    ...
    paper_map = {
        str(p["_id"]): p
        for p in papers_col.find({"_id": {"$in": paper_ids_with_oids}})
    }
...
def _build_attendance_matrix_payload(args):
    ...
    academic_session = _normalise_year(
        args.get("academic_session", "")
    ) or _normalise_year(args.get("academic_year", ""))
    ...
    if to_date:
        ...
        range_end_utc = _local_midnight_to_utc(
            to_local_exclusive, tz_offset_minutes
        )

    courses = sanitise_many(
        get_all_courses(
            ["name", "code", "status", "course_duration", "department"]
        )
    )
    papers = sanitise_many(
        get_all_papers(["name", "code", "semester", "course_id"])
    )
    paper_map = {p["_id"]: p for p in papers}
    ...
    if department_filter:
        for c in courses:
            if (
                _as_text(c.get("department", "")).lower()
    ...
    for paper in papers:
        ...
        if (
            department_filter
            and _as_text(paper.get("course_id")) not in dept_course_ids
        ):
            continue
        ...
        if (
            semester_filter
            and _as_text(paper.get("semester")) != semester_filter
        ):
            continue
    ...
    for profile in profiles:
        ...
        enrolled = [
            _as_text(pid)
            for pid in (profile.get("enrolled_papers") or [])
            if _as_text(pid)
        ]
        ...
        if semester_filter:
            ...
            has_semester_paper = any(
                _as_text((paper_map.get(pid) or {}).get("semester"))
                == semester_filter
                for pid in enrolled
            )
        ...
        candidate_students.append(
            {
                "user_id": user_id,
                "roll_no": _as_text(
                    profile.get("roll_number") or profile.get("reg_number")
                ),
    ...
    for doc in session_docs:
        ...
        subject_code = _as_text(
            paper.get("code") or paper.get("name") or "SUB"
        )
        subject_name = _as_text(
            paper.get("name") or paper.get("code") or "Subject"
        )
        ...
        for sid in doc.get("user_ids") or []:
            ...
            if sid_text:
                date_subject_sessions[compound_key]["present_set"].add(
                    sid_text
                )
        ...
        date_bucket.append(
            {
                ...
                "label": (
                    f"{col['subject_code']} ({col['period_number']})"
                    if col["period_number"]
                    else col["subject_code"]
                ),
            }
        )

    dates = [
        {"date": d, "subjects": grouped_dates[d]}
        for d in sorted(grouped_dates.keys())
    ]
    ...
    if not available_semesters and course_id:
        course_doc = next(
            (c for c in courses if _as_text(c.get("_id")) == course_id), None
        )
        ...
        options_courses.append(
            {
                ...
                "status": _as_text(course.get("status") or "active").lower()
                or "active",
                "course_duration": _to_int(course.get("course_duration"), 0),
            }
        )

    options_courses.sort(
        key=lambda x: (x.get("name") or "", x.get("code") or "")
    )
    ...
    return {
        ...
        "options": {
            ...
            "semesters": sorted(
                available_semesters, key=lambda x: _to_int(x, 0)
            ),
        },
        "meta": {
...
def attendance_matrix_export(user):
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "error": "openpyxl is required for Excel export. Install it in backend requirements."
                }
            ),
            500,
        )
    ...
    for date_entry in dates:
        ...
        for sub in subjects:
            ws.cell(
                ...
                column=col_idx,
                value=_as_text(
                    sub.get("subject_code") or sub.get("subject_name") or "SUB"
                ),
            )
        ...
        ws.cell(
            row=1,
            column=start_col,
            value=_as_text(date_entry.get("date")) or "Date",
        )
    ...
    ws.merge_cells(
        start_row=1,
        start_column=total_attended_col,
        end_row=2,
        end_column=total_attended_col,
    )
    ws.merge_cells(
        start_row=1,
        start_column=total_held_col,
        end_row=2,
        end_column=total_held_col,
    )
    ws.merge_cells(
        start_row=1,
        start_column=percentage_col,
        end_row=2,
        end_column=percentage_col,
    )
    ...
    for i, row in enumerate(rows, start=body_start_row):
        ...
        for j, sub in enumerate(ordered_subjects, start=3):
            value = (
                _as_text(
                    (row.get("cells") or {}).get(sub.get("column_key"), "X")
                )
                or "X"
            )
        ...
        percentage = (
            round((total_attended / total_held) * 100, 2)
            if total_held > 0
            else 0
        )
...
def attendance_matrix_export_pdf(user):
    ...
    title = Paragraph(
        "<b>Official Attendance Matrix Report</b>", styles["Title"]
    )
    ...
    elements.append(
        Paragraph(
            "Authorized Administrator Signature: _______________________",
            styles["Normal"],
        )
    )
...
def dashboard_stats(user):
    ...
    courses = sanitise_many(
        get_all_courses(
            ["name", "code", "year", "status", "department"],
    ...
    profile_query = (
        {"course_id": {"$in": course_ids_with_oids}}
        if (user_dept and course_ids_with_oids)
        else {}
    )
    profiles = list(
        profiles_col.find(
    ...
    if user_dept:
        student_user_ids = [
            _as_text(p.get("user_id")) for p in profiles if p.get("user_id")
        ]
        valid_oids = []
        for u in student_user_ids:
            valid_oids.extend(_id_variants(u))
        student_query["$or"] = [{"_id": {"$in": valid_oids}}]
    ...
    if user_dept:
        ...
        inactive_courses_count = courses_col.count_documents(
            {"department_id": user_dept, "status": "inactive"}
        )
    else:
        active_courses_count = courses_col.count_documents(
            {
    ...
    attendance_count = (
        attendance_col.count_documents(
            {"paper_id": {"$in": all_paper_ids}}
            if user_dept and all_paper_ids
            else {}
        )
        if not (user_dept and not all_paper_ids)
        else 0
    )
    ...
    if started_at:
        ...
        has_tz = (
            iso_started_at.endswith("Z") or "+" in tz_part or "-" in tz_part
        )
...
def monthly_attendance_trend_api(user):
    ...
    if scope_dept_id:
        ...
        courses = list(
            get_collection("academic", "courses").find(
        ...
        papers = list(
            get_collection("academic", "papers").find(
    ...
    # ── Attendance logs from committed sessions only: total records + unique attended students per month ──
    sessions_col = get_collection("attendance", "attendance_sessions")
    ...
    # Attendance logs: total records + unique students per month (from committed sessions only)
    att_pipeline = [
        {"$match": att_query},
        {
            "$lookup": {
        ...
        },
        {
            "$group": {
                "_id": {
                    "year": {"$year": "$session_info.committed_at"},
                    "month": {"$month": "$session_info.committed_at"},
                },
                "total": {"$sum": 1},
    ...
    sess_pipeline = [
        {"$match": sess_query},
        {
            "$group": {
                "_id": {
                    "year": {"$year": "$committed_at"},
                    "month": {"$month": "$committed_at"},
                },
                "sessions": {"$sum": 1},
            }
        },
...
def send_shortage_alerts(user):
    ...
    for profile in profiles:
        ...
        if paper_id_filter:
            enrolled_papers = [
                p for p in enrolled_papers if str(p) == str(paper_id_filter)
            ]
        ...
        for paper_id in enrolled_papers:
            ...
            if pct < threshold:
                ...
                needed_float = (
                    ((threshold_dec * effective_total) - attended) / divider
                    if divider > 0
                    else 0
                )
                classes_needed = max(
