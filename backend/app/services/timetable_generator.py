...
def _score_against_busy_slots(template: Dict[str, Any], busy_slots: List[Dict[str, Any]]) -> int:
    if not busy_slots:
        return 0
    ...
    *,
    ...
    if recess_start_time and recess_end_time:
        ...
        if recess_end_minutes <= recess_start_minutes:
            raise ValueError("recess_end_time must be later than recess_start_time")
    ...
    for day_index, day_name in enumerate(normalized_days):
        ...
        while pointer + class_duration_minutes <= end_minutes:
            ...
            if recess_start_minutes is not None and recess_end_minutes is not None:
                if _overlaps(slot_start, slot_end, recess_start_minutes, recess_end_minutes):
...
def generate_timetable_slots(
    *,
    ...
    if not papers:
        raise ValueError("At least one paper is required to generate timetable")
    ...
    if not slot_templates:
        raise ValueError("No class slots available for the given time boundaries")
    ...
    if max_classes_per_day is not None:
        ...
        if not filtered_templates:
            raise ValueError("No class slots available after applying max_classes_per_day")
    ...
    lecturer_busy_slots = (conflict_context or {}).get("lecturer_busy_slots") or {}
    lecturer_busy_slots = {
        str(key): value if isinstance(value, list) else []
        for key, value in lecturer_busy_slots.items()
    }

    def _candidate_sort_key(paper: dict, template: dict, order_index: int) -> tuple:
    ...
    for idx, template in enumerate(filtered_templates):
        ...
        if assign_paper:
            scored_candidates = sorted(
                ...
                key=lambda item: _candidate_sort_key(item[1], template, item[0]),
            )
            paper = (scored_candidates[0][1] if scored_candidates else None)
        if assign_paper:
            day_assigned_counts[day_index] = day_assigned_counts.get(day_index, 0) + 1
            lecturer_id = str((paper or {}).get("lecturer_id") or "")
            if lecturer_id:
                lecturer_assigned_counts[lecturer_id] = lecturer_assigned_counts.get(lecturer_id, 0) + 1
    ...
    metadata = {
        ...
        "max_classes_per_day": int(max_classes_per_day) if max_classes_per_day is not None else None,
        "assigned_slots": len([slot for slot in generated_slots if slot.get("paper_id")]),
