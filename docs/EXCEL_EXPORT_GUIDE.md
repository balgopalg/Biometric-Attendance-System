...
In addition to client-side table export, the current system also supports server-driven attendance matrix exports and timetable PDF downloads in role-specific pages.
...
✅ **Attendance Matrix Export API**: CSV/XLSX generated by backend for matrix views  
✅ **Timetable PDF Export**: Client-generated PDF snapshots for admin/lecturer/student timetable pages

## Export Modes in the System

### 1) Client-side table export (`excelExport.js`)

- Best for admin list views where filtered data is already in memory.
- Uses the utility patterns described in this guide.

### 2) Attendance matrix backend export

- Endpoints:
  - `GET /api/admin/attendance-matrix/export` (XLSX)
  - `GET /api/admin/attendance-matrix/export-csv` (CSV)
- Required query params:
  - `course_id`
  - `academic_session`
  - `semester`
- Best for canonical attendance exports sourced directly from backend aggregation.

### 3) Timetable PDF export

- Available on timetable pages for admin, lecturer, and student workflows.
- Uses client-side PDF generation from rendered timetable grids.
- Intended for printable weekly schedules and sharing snapshots.
