...
export default function FaceSearchModal({ isOpen, onClose }) {
  ...
  const handleFindFace = async () => {
    ...
    try {
      setLoading(true);
      const res = await api.post('/recognition/find-student', { frame });
      setSearchFrame(frame);
      setMatchedStudent(res.data.student);
      toast.success('Student identified!');
      if (!useFileUpload) stopCamera();
    } catch (err) {
      toast.error(err.response?.data?.error || 'Student not identified');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="glass-card" style={{
        ...
        <div style={{ padding: '24px' }}>
          ...
          <p style={{ fontSize: '0.88rem', color: 'var(--text-muted)', marginBottom: '20px', lineHeight: 1.5 }}>
            {useFileUpload 
              ? 'Upload a clear face photo to identify the student.'
              : "Scan a student's face to identify them and view their details."}
          </p>
            ...
            <div style={{ textAlign: 'left', animation: 'fadeIn 0.3s ease-out' }}>
              <div className="results-container" style={{ 
                ...
                <div className="details-col" style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <h3 style={{ fontSize: '1.2rem', fontWeight: 700, color: 'var(--text-primary)', marginBottom: 4 }}>{matchedStudent.name}</h3>
                  <div className="info-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(80px, auto) 1fr', gap: '4px 12px', fontSize: '0.9rem' }}>
                    <span style={{ color: 'var(--text-muted)' }}>Regd No:</span>
                    <span style={{ fontWeight: 600 }}>{matchedStudent.reg_number}</span>
                    
                    <span style={{ color: 'var(--text-muted)' }}>Dept:</span>
                    <span style={{ fontWeight: 600 }}>{matchedStudent.department}</span>
                    
                    <span style={{ color: 'var(--text-muted)' }}>Course:</span>
                    <span style={{ fontWeight: 600 }}>{matchedStudent.course}</span>
                    
                    <span style={{ color: 'var(--text-muted)' }}>Session:</span>
                    <span style={{ fontWeight: 600 }}>{matchedStudent.academic_session}</span>
                    
                    <span style={{ color: 'var(--text-muted)' }}>Sem:</span>
                    <span style={{ fontWeight: 600 }}>{matchedStudent.current_semester}</span>
                  </div>
                  <div style={{ 
                    fontSize: '0.8rem', 
                    ...
                    Match Confidence: {(matchedStudent.similarity * 100).toFixed(1)}%
                  </div>
                </div>
              </div>
